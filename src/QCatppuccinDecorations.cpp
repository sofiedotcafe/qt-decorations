...
static QMap<QCatppuccinDecorations::ButtonIcon, QString> buttonMap = {
    { QCatppuccinDecorations::CloseIcon, QStringLiteral("window-close-symbolic") },
    { QCatppuccinDecorations::MinimizeIcon, QStringLiteral("window-minimize-symbolic") },
    { QCatppuccinDecorations::MaximizeIcon, QStringLiteral("window-maximize-symbolic") },
    { QCatppuccinDecorations::RestoreIcon, QStringLiteral("window-restore-symbolic") }
};

// Flavor macros
#define Latte QCatppuccinDecorations::Latte
#define Frappe QCatppuccinDecorations::Frappe
#define Macchiato QCatppuccinDecorations::Macchiato
#define Mocha QCatppuccinDecorations::Mocha

// Allow override via compiler
#ifndef CATPPUCCIN_DARK_FLAVOR
#  define CATPPUCCIN_DARK_FLAVOR Mocha
#endif

#ifndef CATPPUCCIN_LIGHT_FLAVOR
#  define CATPPUCCIN_LIGHT_FLAVOR Latte
#endif

// Use directly
constexpr QCatppuccinDecorations::CatppuccinFlavor darkFlavor = CATPPUCCIN_DARK_FLAVOR;
constexpr QCatppuccinDecorations::CatppuccinFlavor lightFlavor = CATPPUCCIN_LIGHT_FLAVOR;

using ColorType = QCatppuccinDecorations::ColorType;

const QMap<ColorType, QColor> QCatppuccinDecorations::catppuccinPalettes[4] = {
    // Latte
    { { Background, QColor("#f5e0dc") },
      { BackgroundInactive, QColor("#f2cdcd") },
      { Foreground, QColor("#4c4f69") },
      { ForegroundInactive, QColor("#6c6f85") },
      { Border, QColor("#dce0e8") },
      { BorderInactive, QColor("#e6e9ef") },
      { ButtonBackground, QColor("#eff1f5") },
      { ButtonBackgroundInactive, QColor("#e6e9ef") },
      { HoveredButtonBackground, QColor("#ccd0da") },
      { PressedButtonBackground, QColor("#b5bfe2") } },
    // Frappe
    { { Background, QColor("#303446") },
      { BackgroundInactive, QColor("#292c3c") },
      { Foreground, QColor("#c6d0f5") },
      { ForegroundInactive, QColor("#a5adce") },
      { Border, QColor("#414559") },
      { BorderInactive, QColor("#51576d") },
      { ButtonBackground, QColor("#414559") },
      { ButtonBackgroundInactive, QColor("#51576d") },
      { HoveredButtonBackground, QColor("#626880") },
      { PressedButtonBackground, QColor("#737994") } },
    // Macchiato
    { { Background, QColor("#24273a") },
      { BackgroundInactive, QColor("#1e2030") },
      { Foreground, QColor("#cad3f5") },
      { ForegroundInactive, QColor("#a5adcb") },
      { Border, QColor("#363a4f") },
      { BorderInactive, QColor("#494d64") },
      { ButtonBackground, QColor("#363a4f") },
      { ButtonBackgroundInactive, QColor("#494d64") },
      { HoveredButtonBackground, QColor("#5b6078") },
      { PressedButtonBackground, QColor("#6e738d") } },
    // Mocha
    { { Background, QColor("#1e1e2e") },
      { BackgroundInactive, QColor("#181825") },
      { Foreground, QColor("#cdd6f4") },
      { ForegroundInactive, QColor("#a6adc8") },
      { Border, QColor("#313244") },
      { BorderInactive, QColor("#45475a") },
      { ButtonBackground, QColor("#313244") },
      { ButtonBackgroundInactive, QColor("#45475a") },
      { HoveredButtonBackground, QColor("#585b70") },
      { PressedButtonBackground, QColor("#6c7086") } }
};

Q_DECL_IMPORT void qt_blurImage(QPainter *p, QImage &blurImage, qreal radius, bool quality,
                                bool alphaOnly, int transposed = 0);

Q_LOGGING_CATEGORY(QCatppuccinDecorationsLog, "qt.qpa.qcatppuccindecorations", QtWarningMsg)
...
QCatppuccinDecorations::QCatppuccinDecorations()
{
#ifdef HAS_QT6_SUPPORT
#  if QT_VERSION >= 0x060000
    qCDebug(QCatppuccinDecorationsLog) << "Using Qt6 version";
#  else
    qCDebug(QCatppuccinDecorationsLog) << "Using Qt5 version with Qt6 backports";
#  endif
#else
    qCDebug(QCatppuccinDecorationsLog) << "Using Qt5 version";
#endif
    ...
    QTimer::singleShot(0, this, &QCatppuccinDecorations::initConfiguration);
}

void QCatppuccinDecorations::initConfiguration()
{
                    ...
                    if (!settings.isEmpty()) {
                        const uint colorScheme =
                                settings.value(QLatin1String("org.freedesktop.appearance"))
                                        ...
                                        .toUInt();
                        qCDebug(QCatppuccinDecorationsLog)
                                << "Parsed color-scheme value:" << colorScheme;
                        updateColors(colorScheme == 1 ? darkFlavor : lightFlavor);
                        ...
                        if (!buttonLayout.isEmpty()) {
                            qCDebug(QCatppuccinDecorationsLog)
                                    << "Parsed button-layout:" << buttonLayout;
                        ...
                        const QString titlebarFont =
                                settings.value(QLatin1String("org.gnome.desktop.wm.preferences"))
                                        ...
                                        .toString();
                        qCDebug(QCatppuccinDecorationsLog)
                                << "Parsed titlebar-font:" << titlebarFont;
...
void QCatppuccinDecorations::updateColors(QCatppuccinDecorations::CatppuccinFlavor flavor)
{
    qCDebug(QCatppuccinDecorationsLog) << "Changing color scheme to Catppuccin flavor" << flavor;

    m_colors = catppuccinPalettes[flavor];
    forceRepaint();
}
...
QString getIconSvg(const QString &iconName)
{
    ...
    qCDebug(QCatppuccinDecorationsLog) << "Icon themes:" << themeNames;
    ...
    qCWarning(QCatppuccinDecorationsLog) << "Failed to find an svg icon for" << iconName;

    return QString();
}

void QCatppuccinDecorations::updateIcons()
{
...
void QCatppuccinDecorations::updateTitlebarLayout(const QString &layout)
{
    qCDebug(QCatppuccinDecorationsLog) << "Changing titlebar layout to" << layout;
...
void QCatppuccinDecorations::settingChanged(const QString &group, const QString &key,
                                            const QDBusVariant &value)
{
    qCDebug(QCatppuccinDecorationsLog) << "SettingChanged signal received:"
                                       << "Group:" << group << "Key:" << key;
    ...
    if (group == QLatin1String("org.gnome.desktop.wm.preferences")
        ...
        qCDebug(QCatppuccinDecorationsLog) << "Updated button-layout from signal:" << layout;
        ...
        qCDebug(QCatppuccinDecorationsLog) << "Updated color-scheme from signal:" << colorScheme;
        updateColors(colorScheme == 1 ? darkFlavor : lightFlavor);
    }
}

QRectF QCatppuccinDecorations::buttonRect(Button button) const
{
    int xPos;
    int yPos;
...
#ifdef HAS_QT6_SUPPORT
QMargins QCatppuccinDecorations::margins(MarginsType marginsType) const
{
...
#else
QMargins QCatppuccinDecorations::margins() const
{
...
void QCatppuccinDecorations::paint(QPaintDevice *device)
{
    // FIX: Core structural safety guard block to abort drawing frames if wayland elements are not
    // ready
    if (!window() || !waylandWindow()) {
        return;
    }
    ...
    // Window title
    {
        ...
        // FIX: Replaced waylandWindow()->windowTitle() to avoid thread lock collisions
...
static void renderFlatRoundedButtonFrame(QCatppuccinDecorations::Button button, QPainter *painter,
...
static void renderButtonIcon(QCatppuccinDecorations::ButtonIcon buttonIcon, QPainter *painter,
                             const QRect &rect)
{
...
static QCatppuccinDecorations::ButtonIcon
iconFromButtonAndState(QCatppuccinDecorations::Button button, bool maximized)
{
    if (button == QCatppuccinDecorations::Close)
        return QCatppuccinDecorations::CloseIcon;
    else if (button == QCatppuccinDecorations::Minimize)
        return QCatppuccinDecorations::MinimizeIcon;
    else if (button == QCatppuccinDecorations::Maximize && maximized)
        return QCatppuccinDecorations::RestoreIcon;
    else
        return QCatppuccinDecorations::MaximizeIcon;
}

void QCatppuccinDecorations::paintButton(Button button, QPainter *painter)
{
#ifdef HAS_QT6_SUPPORT
    const Qt::WindowStates windowStates = waylandWindow()->windowStates();
    const bool active = windowStates & Qt::WindowActive;
...
bool QCatppuccinDecorations::clickButton(Qt::MouseButtons b, Button btn)
{
...
bool QCatppuccinDecorations::doubleClickButton(Qt::MouseButtons b, const QPointF &local,
...
bool QCatppuccinDecorations::handleMouse(QWaylandInputDevice *inputDevice, const QPointF &local,
...
#if QT_VERSION >= 0x060000
bool QCatppuccinDecorations::handleTouch(QWaylandInputDevice *inputDevice, const QPointF &local,
...
QRect QCatppuccinDecorations::windowContentGeometry() const
{
#ifdef HAS_QT6_SUPPORT
...
void QCatppuccinDecorations::forceRepaint()
{
    // FIX: Comprehensive validation check. If the decoration interface isn't fully linked
    // into the running window manager hierarchy yet, abort before causing thread collisions.
    if (!window() || !waylandWindow() || !waylandWindow()->decoration()) {
        return;
    }
    ...
    // Request event update on the window loop safely
    window()->requestUpdate();
}

void QCatppuccinDecorations::processMouseTop(QWaylandInputDevice *inputDevice, const QPointF &local,
...
void QCatppuccinDecorations::processMouseBottom(QWaylandInputDevice *inputDevice,
...
void QCatppuccinDecorations::processMouseLeft(QWaylandInputDevice *inputDevice,
                                              const QPointF &local, Qt::MouseButtons b,
                                              Qt::KeyboardModifiers mods)
{
    Q_UNUSED(local)
...
void QCatppuccinDecorations::processMouseRight(QWaylandInputDevice *inputDevice,
                                               const QPointF &local, Qt::MouseButtons b,
                                               Qt::KeyboardModifiers mods)
{
    Q_UNUSED(local)
...
bool QCatppuccinDecorations::updateButtonHoverState(Button hoveredButton)
{
